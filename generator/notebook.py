...
import re
import time
from .helper import LEETCODE, PROBLEMS_JSON, NOTEBOOK_TEMPLATE_JSON, get_folder_for
...
def generate(question_id, question_info):
    ...
    file_path = os.path.join(directory, '{id}.{title}.ipynb'.format(id=question_id, title=question_info['titleSlug']))
...
def generate_notebook(question_id):
    with requests.Session() as s:
        ...
        if not os.path.exists(PROBLEMS_JSON) or ((time.time() - os.path.getmtime(PROBLEMS_JSON)) // (60*60*24*30)) > 3:
